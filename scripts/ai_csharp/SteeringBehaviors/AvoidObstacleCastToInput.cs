...
namespace ISIS.Minds.SteeringBehaviors {
    public class AvoidObstacleCastToInput : AvoidObstacleSebLagueRay {
        public AvoidObstacleCastToInput(RigidBody craft, Vector3 craftExtents, float predectionTimeSeconds = 5, IEnumerable<RID> obstacleExculsionList = null):
        ...
        protected override(bool collisionPredicted, Vector3 detectionVector) PredictCollision(Transform currentTransform, CraftStateWrapper currentState) {
            var castTo = currentState.LinearInput;
            castTo = castTo.Normalized();

            castTo *= currentState.LinearVelocty.Length();
